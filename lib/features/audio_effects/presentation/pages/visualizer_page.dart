...
class _VisualizerPageState extends ConsumerState<VisualizerPage> {
  ...
  Future<void> _start() async {
    ...
    if (!status.isGranted) {
      if (mounted) {
        setState(() {
          _message =
              'Microphone permission is required by Android for playback '
              'visualization. Flute does not record or save microphone audio.';
        });
      }
      return;
    }
    ...
    if (!started) {
      if (mounted) {
        setState(() {
          _message =
              'Start playing a song, then reopen the visualizer. '
              'This device may not support the Android visualizer effect.';
        });
      }
      return;
    }
  ...
  void dispose() {
    _timer?.cancel();
    unawaited(
      ref.read(audioEffectsServiceProvider).stopVisualizer(),
    );
    super.dispose();
  }
